/**
 * First-party tracking library for Growthsuite.
 *
 ...
 *
 * Identical engine to lalloronacantina; differs only in default `site`.
 *
 * Usage from _app.js:
 *   import { initTracker, trackPageview } from '../lib/tracker';
 *   useEffect(() => { initTracker({ site: 'growthsuite' }) }, []);
...
/**
 * Eventos disparados ANTES de que initTracker corriera.
 *
 * Causa raíz: React ejecuta los efectos de los HIJOS antes que los del padre, así que un
 * componente que llama trackEvent en su useEffect corre antes del initTracker de _app.js.
 * Hasta ahora esos eventos se descartaban en silencio — justo los más valiosos, porque son
 * los de "el usuario acaba de llegar/abrir X".
 *
 * Se encolan y se envían al inicializar, en vez de perderse. El tope evita que una página
 * sin tracker acumule memoria sin fin.
 */
const pending = [];
...
export function initTracker({ site, endpoint, debug } = {}) {
  ...
  /* Y todo lo que se disparó antes de este punto (efectos de componentes hijos). */
  while (pending.length) {
    trackPageview(pending.shift());
  }
}
...
export function trackPageview(opts = {}) {
  if (typeof window === 'undefined') return;
  if (!initialized) {
    /* Todavía no arranca el tracker: se guarda y sale en cuanto initTracker corra. */
    if (pending.length < MAX_PENDING) pending.push(opts);
    return;
  }
 ...
 *
 *   buildBookingWidgetUrl('http://localhost:5174', 'growthsuite-demos', 'demo-agenda')
 *   → 'http://localhost:5174/?slug=growthsuite-demos&type=demo-agenda&lead_uid=ld_xyz&utm_source=...'
 *
...
export function buildCalendlyUrl(baseUrl) {
   ...
   * Growthsuite leads from Llorona leads when both feed into the same API */
